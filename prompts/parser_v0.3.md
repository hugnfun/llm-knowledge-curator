...
│    │
│    补充性验证(三条必须同时满足才命中):
│    (i) 能用一句话说出作者的核心论点/框架/洞察
│    (ii) 能说出一个具体的、不同于作者的补充角度
│         (不是“我也可以用AI做XX”这种泛泛联想)
│    (iii) 内容本身有原创观点输出,不是纯教程/资讯/清单
│    │
│    以下情况不算命中,直接走 Step 5/6:
│    - 纯教程/SOP/操作指南:即使话题相关也走 Step 5
│    - 纯工具推荐/清单:即使提到在用工具也走 Step 3 或 Step 5
│    - 行业市场分析/数据报告:除非直接讨论用户在做的业务,否则 Step 6
│    - 泛泛观点/励志/鸡汤:即使提到AI也走 Step 6
│    │
...
### 样本 8 - Step 4 不命中:营销教程即使话题相关也走 Step 5
```
输入:公众号爆文写作指南:标题用数字+情绪词,开头借热门话题引入,中间分点阐述,结尾总结
输出:
{
  "verdict": "asset",
  "category": "教程",
  "trigger": "",
  "reason": "纯写作教程,虽涉及营销关注点但无原创论点,补充性验证不通过",
  "confidence": "high",
  "priority": "normal"
}
```
> 说明:内容看起来跟"营销/增长"关注点重合,但作者只是在教方法,没有提出可争议的论点。补充性验证 (i) 失败--说不出一个核心论点。走 Step 5。

### 样本 9 - Step 3 命中但无观点 -> asset(工具教程不算 seed)
```
输入:Claude Code 项目结构最佳实践:CLAUDE.md 应该包含目标、地图、规则、工作流四个部分
输出:
{
  "verdict": "asset",
  "category": "工具",
  "trigger": "",
  "reason": "讨论Claude Code用法但属教程无原创批判,Step 3工具元话题标记高优先级",
  "confidence": "high",
  "priority": "high"
}
```
> 说明:提到了在用工具 Claude Code,命中 Step 3。但内容是使用教程,不含原创观点/批判/机制拆解。Step 3 内部判定走 asset + priority=high。

### 样本 10 - Step 4 不命中:AI 相关但泛泛观点走 Step 6
```
输入:30岁的人应该抓住AI机会,先用上Claude,每天看AI内容,不必焦虑先上车再说
输出:
{
  "verdict": "archive",
  "category": "",
  "trigger": "",
  "reason": "泛泛AI励志观点无具体论点,补充性验证不通过,与用户深度关注点不匹配",
  "confidence": "high",
  "priority": "normal"
}
```
> 说明:提到了AI和Claude,看起来跟用户关注点重合。但内容是泛泛励志,没有具体论点/框架/洞察。补充性验证 (i)(ii)(iii) 全部失败。走 Step 6。
...
- **v0.3.1 (2026-07-24)**:
  - Step 4 增加补充性验证(三条必须同时满足)和四类排除规则,解决 LLM 把话题沾边当可补充的问题
  - 新增 3 条反例 few-shot(样本 8/9/10):营销教程->asset、工具教程->asset、泛泛AI观点->archive
  - 基于 40 条 golden set 评测发现:v0.3 seed 精度仅 33%(18 条误判全是 asset/archive->seed),本次修改针对此问题
